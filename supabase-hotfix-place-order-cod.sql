-- HOTFIX: place_order per-product COD enforcement + food-aware order_type
--
-- Adds COD eligibility check per product during validation.
-- If payment method is COD and any product has cod_enabled = false,
-- the entire order is rejected with a clear message.
--
-- This enforces the mixed-cart rule: if ANY product is COD-ineligible,
-- the entire COD order is rejected.
--
...
--
-- Based on supabase-hotfix-place-order-coupon.sql (authoritative base).
-- Requires manual review; DO NOT auto push.
