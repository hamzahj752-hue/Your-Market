...
export default function FeaturedProducts() {
  ...
  const [products, setProducts] = React.useState<BriefProduct[]>(
    defaultFeaturedProducts as BriefProduct[]
  );
  ...
  useEffect(() => {
    ...
    fetchFeaturedProducts().then((cms) => {
      if (!active) return;
      if (cms.length > 0) setProducts(cms);
    });
    return () => {
      active = false;
    };
  ...
  const handleAddToCart = (p: BriefProduct) => {
    addToCart({
      id: p.id,
      name: p.name,
      price: p.price,
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {products.map((p, i) => (
            <div key={p.id} data-idx={i} className="prod-card animate-on-scroll">
