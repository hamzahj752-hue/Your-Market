...
export default function DealsSection() {
  ...
  const [deals, setDeals] = useState<BriefDeal[]>(defaultDealProducts as BriefDeal[]);
  ...
  useEffect(() => {
    ...
    Promise.all([fetchDeals(), fetchPromoBanners()]).then(([cmsDeals, cmsPromos]) => {
      if (!active) return;
      if (cmsDeals.length > 0) setDeals(cmsDeals);
      setPromos(cmsPromos);
    });
    return () => {
      active = false;
  ...
  return (
    <section className="section-pad bg-white" aria-labelledby="deals-heading">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          {deals.map((p) => (
            <Link key={p.id} href="/products" className="block group">
              <div className="bg-muted/30 rounded-2xl overflow-hidden card-shadow product-card-hover border border-border/50">
                ...
                <div className="p-4">
                  <h3 className="text-sm font-700 text-foreground leading-snug mb-2 line-clamp-2">
                    {p.name}
                  </h3>
                  <div className="flex items-baseline gap-2 mb-1">
                    <span className="text-base font-800 price-deal">
                      रू{p.price.toLocaleString()}
                    </span>
        ...
        {promos.length > 0 ? (
          promos.map((promo) => (
            <div key={promo.id} className="mt-8 rounded-3xl overflow-hidden relative h-40 md:h-52">
              {promo.image_url && (
                <img
                  src={promo.image_url}
              ...
              <div className="relative z-10 h-full flex items-center justify-between px-8 md:px-12">
                ...
                {promo.cta_text && promo.cta_url && promo.cta_url.startsWith('/') && (
                  <Link href={promo.cta_url} className="hidden md:block">
